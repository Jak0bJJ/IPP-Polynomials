...
/**
 * Mnoży jednomian przez pewną stałą
 * @param[in] p : wielomian będący stałą @f$p@f$
...
/**
 * Standaryzuje wielomian, zmieniając go przy tym.
 * @param[in] p : Zestandaryzowany wielomian @f$p@f$
 */
...
/**
 * Wypisuje na standardowe wyjście jednomian @f$m@f$
...
/**
 * Wypisuje na standardowe wyjście wielomian @f$p@f$
 * @param[in] m : wielomian @f$p@f$
 * @return Wielomian
 */
void PrintPoly (Poly *p);

/**
 * Jednomian przechowujący informacje o błędzie
 * @return Jednomian typu Error
 */
static inline Mono MonoErr ()
{
    return (Mono) {.p=PolyZero(), .exp=-1};
}

/**
 * Wielomian przechowujący informacje o błędzie
 * @return Wielomian typu Error
 */
static inline Poly PolyErr ()
{
    Poly p;
...
/**
 * Sprawdza, czy jednomian przechowuje informacje o błędzie
 * @param[in] m : jednomian
 * @return Czy jednomian jest typu Error?
 */
static inline bool MonoIsErr (Mono *m)
{
    return PolyIsZero(&m->p) && m->exp==-1;
}
/**
 * Sprawdza, czy wielomian przechowuje informacje o błędzie
 * @param[in] p : wielomian
 * @return Czy wielomian jest typu Error?
 */
static inline bool PolyIsErr (Poly *p)
{
    return (!PolyIsCoeff(p)) && (MonoIsErr(&p->arr[0]));
}

/**
 * Tworzy jednomian na podstawie tekstu @p line
 * @param[in] line : tekst
 * @return Jednomian wyczytany z tekstu @p line
 */
Mono MonoFromStr (char *line);

/**
 * Tworzy wielomian na podstawie tekstu @p line
 * @param[in] line : tekst
 * @return Wielomian wyczytany z tekstu @p line
 */
 ...
 * oznaczone odpowiednio x0, x1, x2, …, xl−1. Wynikiem złożenia jest wielomian
 * p(q0,q1,q2,…), czyli wielomian powstający przez podstawienie w wielomianie p
 * pod zmienną xi wielomianu qi dla i=0,1,2,…,min(k,l)−1. Jeśli k<l, to pod zmienne
 * xk, …, xl−1 podstawiamy zera.
